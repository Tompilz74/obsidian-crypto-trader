...
function computePathMomentum(prices: number[]): PathMomentum {
  const clean = prices.filter((p) => typeof p === "number" && isFinite(p) && p > 0);
  if (clean.length < 2) {
    return {
      path1h: 0,
  ...
  const path1h = recentPathPct(clean, 1);
  const path4h = recentPathPct(clean, 4);
  const path8h = recentPathPct(clean, 8);
  const path24h = recentPathPct(clean, Math.min(24, clean.length - 1));
  const recent = clean.slice(Math.max(1, clean.length - 13));
  const deltas = recent.map((price, i) => (i === 0 ? 0 : price - recent[i - 1])).slice(1);
  const greenRatio = deltas.length ? deltas.filter((d) => d > 0).length / deltas.length : 0;
  const high12 = Math.max(...clean.slice(Math.max(0, clean.length - 12)));
  const last = clean[clean.length - 1];
  const pullbackPct = high12 > 0 ? ((last - high12) / high12) * 100 : 0;
  const prior4hPace = Math.max(0, path8h - path4h) / 4;
  const current4hPace = Math.max(0, path4h) / 4;
  const slopeLift = current4hPace - prior4hPace;
  const stairScore = Math.round(clamp(
    40 +
      Math.max(0, path4h) * 5 +
      Math.max(0, path8h) * 2.2 +
      Math.max(0, slopeLift) * 18 +
      greenRatio * 22 +
      Math.max(-10, pullbackPct) * 1.4,
    0,
    100
  ));
  const verticalRisk = Math.round(clamp(
    Math.max(0, path1h - 2.8) * 14 +
      Math.max(0, path4h - 9) * 6 +
      Math.max(0, path24h - 28) * 1.8 +
      (greenRatio > 0.82 ? 10 : 0) +
      (pullbackPct > -0.35 ? 8 : 0),
    0,
    100
  ));
  const label =
    verticalRisk >= 65
      ? "VERTICAL"
      : stairScore >= 76 && slopeLift > 0.15
        ? "LIFTING"
        : stairScore >= 62
          ? "BUILDING"
          : "FLAT";

  return { path1h, path4h, path8h, path24h, slopeLift, greenRatio, pullbackPct, stairScore, verticalRisk, label };
}
...
function loadSimRevolutXMode() {
  try {
    const stored = localStorage.getItem(LS_SIM_REVOLUT_X_MODE);
...
export default function App() {
      ...
      .map((s) => {
        ...
        const momentumScore = clamp(
          42 +
            ...
            Math.max(0, move4h) * 3 +
            Math.max(0, ch24) * 0.9 +
            (pathAware ? path.stairScore * 0.18 + Math.max(0, path.path4h) * 2.4 : 0),
          0,
          100
        );
        ...
        const peakRiskScore = Math.round(clamp(
          spike6h * 6 +
            ...
            path.verticalRisk * 0.35 -
            (pathStillHasLegs ? 22 : 0) -
            (stairLaunch ? 10 : 0) +
            (rollingOver ? 30 : 0) +
            (latestWeak ? 18 : 0),
          0,
        ...
        const legsScore = Math.round(clamp(
          58 +
            ...
            Math.max(0, move4h) * 2 +
            (pathAware ? path.stairScore * 0.24 + Math.max(0, path.slopeLift) * 12 - path.verticalRisk * 0.16 : 0) -
            peakRiskScore * 0.55 +
            (liquidityOk ? 8 : -10) -
            (rollingOver ? 24 : 0) -
            (latestWeak ? 16 : 0),
        ...
        const speedScore = Math.round(clamp(tapeRatePctPerHour * 18 + Math.max(0, path.path4h) * 2, 0, 100));
        const accelerationScore = Math.round(clamp(48 + acceleration * 30 + (fastMove > 0 ? 8 : -16), 0, 100));
        const continuationScore = Math.round(clamp(legsScore * 0.5 + path.stairScore * 0.32 + Math.max(0, path.slopeLift) * 18 + (continuationOk ? 14 : -18), 0, 100));
        const riskControlScore = Math.round(clamp(100 - peakRiskScore - (rollingOver ? 24 : 0) - (dumpRisk ? 22 : 0) - (path.label === "VERTICAL" ? 16 : 0), 0, 100));
        const liquidityScore = Math.round(clamp(42 + Math.log10(Math.max(1, s.volFactor)) * 34 + (price > 0 ? 12 : -30), 0, 100));
        const gradeScore = Math.round(clamp(
          speedScore * 0.24 +
            accelerationScore * 0.18 +
            continuationScore * 0.26 +
            riskControlScore * 0.22 +
            liquidityScore * 0.1 -
            structurePenalty * 0.7,
          0,
          100
        ));
        const gradeComponents = {
          ...
          risk: riskControlScore,
          liquidity: liquidityScore,
        };
        const targetPotentialPct = clamp(
          tapeRatePctPerHour * 0.85 +
            Math.max(0, acceleration) * 0.65 +
            Math.max(0, path.path4h) * 0.18 +
            Math.max(0, path.slopeLift) * 0.55,
          0,
          8
        );
        const gradeMultiplier = clamp((gradeScore - 38) / 42, 0, 1.25);
        const expectedMovePct = clamp(targetPotentialPct * gradeMultiplier, 0, 7.5);
        const moveBand: ScalpSignal["moveBand"] =
          tooLate || dumpRisk || peakRisk || riskControlScore < 42
            ? "AVOID"
            : gradeScore >= 86 && expectedMovePct >= 4.8 && speedScore >= 72 && continuationScore >= 72 && riskControlScore >= 62
              ? "5%+"
              : gradeScore >= 74 && expectedMovePct >= 2.7 && speedScore >= 55 && continuationScore >= 62 && riskControlScore >= 55
                ? "3%"
                : gradeScore >= 58 && expectedMovePct >= 0.9 && !rollingOver && riskControlScore >= 48
                  ? "1-2%"
                  : "WATCH";
        const gradeLabel =
        ...
        return { symbol: s.symbol, price, burstScore, gradeScore, gradeComponents, legsScore, peakRiskScore, action, tone, gradeLabel, gradeDetail, expectedMovePct, moveBand, movementRatePctPerHour: tapeRatePctPerHour, timeToTarget, speedLabel, legsLabel, stop, target, holdWindow, suggestedUsd, reasons, warnings, setup: s };
      })
      .filter((s) => s.price > 0)
      .filter((s) => !hideFlatCoins || s.action !== "NO_LIQUIDITY")
      .sort((a, b) => b.gradeScore - a.gradeScore || b.burstScore - a.burstScore)
      .slice(0, 20);
    ...
    for (const signal of scalpSignals.slice(0, 12)) {
      ...
      if (freshPct >= alertSettings.minFastMovePct && legsOk && signal.peakRiskScore < 58) {
        ...
        alerts.push({
          ...
          detail: `${freshPct.toFixed(2)}% fresh 1h move. ${signal.gradeLabel} ${signal.moveBand} · grade ${signal.gradeScore}/100 · est ${signal.expectedMovePct.toFixed(2)}% · ${signal.timeToTarget}. ${signal.action === "SCALP_TEST" ? "Scalp-confirmed" : "Watch only until confirmation"}.`,
          tone: "#047857",
      ...
      if (signal.action === "SCALP_TEST" && signal.burstScore >= alertSettings.minBurstScore && legsOk) {
        ...
        if (!seen.has(id)) {
          ...
          alerts.push({
            ...
            detail: `${signal.gradeLabel} ${signal.moveBand} · grade ${signal.gradeScore}/100 · est ${signal.expectedMovePct.toFixed(2)}% · ${signal.timeToTarget}. ${signal.speedLabel} · ${signal.legsLabel}. Hold window: ${signal.holdWindow}`,
            tone: signal.tone,
  ...
  const estimateSimExit = useCallback((position: SimPosition, exit: number) => {
    ...
    const entryFeeUsd = position.entryFeeUsd ?? 0;
    const exitFeeUsd = position.platform === "REVOLUT_X" ? grossValue * (REVOLUT_X_TAKER_FEE_PCT / 100) : 0;
    ...
    return {
      ...
      totalFeesUsd,
      pnlUsd,
      pnlPct: position.notionalUsd > 0 ? (pnlUsd / position.notionalUsd) * 100 : 0,
      cashReturn: grossValue - exitFeeUsd,
    };
  }, []);
  const simOpenPnl = simState.positions.reduce((acc, p) => {
    const price = getSimPrice(p.symbol) || p.entry;
    return acc + estimateSimExit(p, price).pnlUsd;
  }, 0);
  const simEquity = simState.cashUsd + simState.positions.reduce((acc, p) => {
    const price = getSimPrice(p.symbol) || p.entry;
    return acc + estimateSimExit(p, price).cashReturn;
  }, 0);
  ...
  const simSelectedHoldingPnl = simSelectedHolding
    ? estimateSimExit(simSelectedHolding, getSimPrice(simSelectedHolding.symbol) || simSelectedHolding.entry).pnlUsd
    : 0;
  ...
  useEffect(() => {
    setSimState((s) => {
      ...
      for (const position of s.positions) {
        ...
        const exitResult = estimateSimExit(position, price);
        cashToReturn += exitResult.cashReturn;
        closed.push({
          ...position,
          closedAtIso,
          exit: price,
          exitFeeUsd: exitResult.exitFeeUsd,
          totalFeesUsd: exitResult.totalFeesUsd,
  ...
  }, [estimateSimExit, getSimPrice, market]);
  ...
  const simBuyAllowed =
    ...
    simTakeProfitValid &&
    simTicketNotional > 0 &&
    simCashRequired <= simState.cashUsd;
  ...
  const holdingReviews = useMemo(() => {
    ...
    return simState.positions.map((p) => {
      ...
      const exitResult = estimateSimExit(p, now);
      const value = exitResult.cashReturn;
      const pnlUsd = exitResult.pnlUsd;
  ...
  }, [estimateSimExit, getSimPrice, scannerBest, setups, simEquity, simState.cashUsd, simState.positions, simState.startingCashUsd]);
    ...
    try {
      ...
      const holdings = simState.positions.map((p) => {
        ...
        return {
          ...
          entry: p.entry,
          now,
          valueUsd: exitResult.cashReturn,
          pnlUsd: exitResult.pnlUsd,
          pnlPct: exitResult.pnlPct,
          thesis: p.thesis,
        };
  ...
  const buySimCrypto = (symbol = simSymbol, buyUsd = Number(simBuyUsd)) => {
    ...
    const entryFeeUsd = simRevolutXMode ? notional * (REVOLUT_X_TAKER_FEE_PCT / 100) : 0;
    const cashRequired = notional + entryFeeUsd;
    if (cashRequired > simState.cashUsd) {
      alert("Buy amount plus simulated fee is larger than your simulator cash.");
      return;
    }
    ...
    setSimState((s) => ({ ...s, cashUsd: s.cashUsd - cashRequired, positions: [position, ...s.positions] }));
    setSimSymbol(symbol);
    setFocusSymbol(symbol);
    if (price) {
  ...
  const sellSimHolding = (id: string) => {
    setSimState((s) => {
      ...
      const exitResult = estimateSimExit(position, exit);
      const closed: SimClosedTrade = {
        ...position,
      ...
      return {
        ...s,
        cashUsd: s.cashUsd + exitResult.cashReturn,
  ...
  const Simulator = () => (
    <>
      <div style={{ ...panel, marginTop: 14 }}>
        ...
        <div style={{ ...statCard, marginTop: 12, background: simRevolutXMode ? "#eef7f5" : "#ffffff", borderColor: simRevolutXMode ? "#0f766e" : "#e2e8f0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <div>
              <div style={{ fontWeight: 950, color: simRevolutXMode ? "#0f766e" : "#111827" }}>
                Revolut X simulator mode {simRevolutXMode ? "ON" : "OFF"}
              </div>
              <div style={{ ...subtle, marginTop: 5 }}>
                {simRevolutXMode
            ...
            scalpSignals.slice(0, 8).map((signal) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ ...pill, color: signal.tone }}>{signal.gradeScore}/100</span>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0, 1fr))", gap: 4, marginTop: 8 }}>
                  {[
                    ["Spd", signal.gradeComponents.speed],
                    ["Acc", signal.gradeComponents.acceleration],
                    ["Leg", signal.gradeComponents.continuation],
                    ["Risk", signal.gradeComponents.risk],
                    ["Liq", signal.gradeComponents.liquidity],
                  ].map(([label, value]) => (
                    <div key={label} style={{ ...pill, padding: "4px 5px", justifyContent: "center", color: Number(value) >= 65 ? "#047857" : Number(value) >= 48 ? "#92400e" : "#b91c1c" }}>
                      {label} {value}
                    </div>
                  ))}
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setSimThesis(`Quick scalp ${signal.gradeLabel} ${signal.moveBand}: grade ${signal.gradeScore}/100, estimated move ${signal.expectedMovePct.toFixed(2)}% over ${signal.timeToTarget}, rate ${signal.movementRatePctPerHour.toFixed(2)}%/h. Components speed ${signal.gradeComponents.speed}, acceleration ${signal.gradeComponents.acceleration}, continuation ${signal.gradeComponents.continuation}, risk ${signal.gradeComponents.risk}, liquidity ${signal.gradeComponents.liquidity}. ${signal.speedLabel} ${signal.legsLabel}: peak risk ${signal.peakRiskScore}/100. ${signal.reasons.join(" ")} ${signal.warnings.join(" ")}`);
                  }}
                >
                  Load Scalp Ticket
                </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "0.85fr 1.15fr", gap: 14, marginTop: 14 }}>
        <div style={panel}>
            ...
            {simTradeMode === "SCALP" && selectedScalpSignal && (
              <div style={{ ...statCard, marginTop: 10, background: "#ffffff", borderColor: selectedScalpSignal.action === "SCALP_TEST" ? "#99f6e4" : "#fed7aa" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0, 1fr))", gap: 5, marginTop: 8 }}>
                  {[
                    ["Spd", selectedScalpSignal.gradeComponents.speed],
                    ["Acc", selectedScalpSignal.gradeComponents.acceleration],
                    ["Leg", selectedScalpSignal.gradeComponents.continuation],
                    ["Risk", selectedScalpSignal.gradeComponents.risk],
                    ["Liq", selectedScalpSignal.gradeComponents.liquidity],
                  ].map(([label, value]) => (
                    <div key={label} style={{ ...pill, padding: "4px 5px", justifyContent: "center", color: Number(value) >= 65 ? "#047857" : Number(value) >= 48 ? "#92400e" : "#b91c1c" }}>
                      {label} {value}
                    </div>
          ...
          <div style={{ marginTop: 10 }}>
            ...
            <div style={{ ...statCard, marginTop: 8, background: simRevolutXMode ? "#f8fafc" : "#ffffff" }}>
              <div style={{ fontWeight: 950, color: simRevolutXMode ? "#0f766e" : "#64748b" }}>
          ...
          <div style={{ ...subtle, marginTop: 10 }}>
            {!simCanBuySelected
              ...
              : !simStopLossValid || !simTakeProfitValid
                ...
                : simCashRequired > simState.cashUsd
                  ? `Buy amount plus estimated fee (${fmtUsd(simCashRequired)}) is above simulator cash.`
                : simTradeWarningActive
                  ? `${simTradeMode === "SCALP" ? "Scalp warning" : "AI warning"}: ${simBuyBlocker} You can still buy in the simulator.`
                  : `Buying uses the current market price${simRevolutXMode ? " with Revolut X fee simulation" : ""}. Time left: ${simDaysLeft.toFixed(1)} days.`}
          </div>
        </div>
                    ...
                    return (
                      <tr key={p.id} style={{ background: "#f8fafc", verticalAlign: "top" }}>
                        <td style={{ padding: "10px 8px", fontWeight: 950 }}>
                          ...
                          {p.platform === "REVOLUT_X" && <div style={{ ...pill, marginTop: 6, marginLeft: 6, display: "inline-block", color: "#0f766e" }}>REV X</div>}
                        </td>
